...
각 슬롯의 수집 구간은 `[직전 실행 슬롯의 종료 시각, 이번 슬롯의 종료 시각]`이다. 06:10 → 09:50 → 14:05 → 21:20 → (다음날) 06:10 으로 끝과 시작이 정확히 붙어 어떤 발행 시각도 두 번 수집되지 않는다. 토요일 발송 생략은 `prev_run_day` 앵커가 흡수하므로(일요일이면 금요일로 2일 소급) 요일 특례가 따로 없다.
